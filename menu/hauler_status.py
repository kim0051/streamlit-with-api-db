...
def filterData(area, loader, series, start_date, end_date):
    ...
    query = "SELECT * FROM hauler_statuses where date >= '" + start_date.strftime("%Y-%m-%d") + "' and date <= '" + end_date.strftime("%Y-%m-%d") + "'"
    if(area != 'ALL'):
        query += " and area = '" + area + "'"
    if(loader != 'ALL'):
    ...
    # Chart by area
    query_chart1 = "SELECT area, SUM(CAST(" + series + " AS FLOAT)) as "+series+" FROM hauler_statuses where date >= '" + start_date.strftime("%Y-%m-%d") + "' and date <= '" + end_date.strftime("%Y-%m-%d") + "' GROUP BY area"
    ...
    # Chart by date
    query_chart2 = "SELECT date, SUM(CAST(" + series + " AS FLOAT)) as "+series+" FROM hauler_statuses where date >= '" + start_date.strftime("%Y-%m-%d") + "' and date <= '" + end_date.strftime("%Y-%m-%d") + "' GROUP BY date"
    ...
    # Chart by equipment
    query_chart3 = "SELECT EQUIPMENT, SUM(CAST(" + series + " AS FLOAT)) as "+series+" FROM hauler_statuses where date >= '" + start_date.strftime("%Y-%m-%d") + "' and date <= '" + end_date.strftime("%Y-%m-%d") + "' GROUP BY EQUIPMENT"
...
area = pd.read_sql("SELECT DISTINCT area FROM hauler_statuses", conn)
equipment = pd.read_sql("SELECT DISTINCT EQUIPMENT FROM hauler_statuses", conn)
all = pd.Series(['ALL'])
